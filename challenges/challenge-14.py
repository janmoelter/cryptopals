...
_random_salt = random_bytes(length=random.randint(1, 16))

def encryption_oracle(_bytes):
	_unknown_bytes = _input
	return ECB128_encryption(AES128_block_cipher, _random_salt + _bytes + _unknown_bytes, _key_bytes)
...
for n in range(_block_length):
	_enc_bytes = encryption_oracle(bytearray(2*_block_length + n))
	if _enc_bytes[1*_block_length:(1*_block_length + _block_length)] == _enc_bytes[2*_block_length:(2*_block_length + _block_length)]:
		_salt_length = _block_length - n
		break
print('Salt length: ', _salt_length)
if not _salt_length == len(_random_salt): sys.exit(1)

_0_padded_enc_bytes = dict()
for n in range(_block_length):
	_0_padded_enc_bytes[n] = encryption_oracle(bytearray(_block_length - _salt_length + n))[_block_length:]
...
for n in range(_number_unknown_bytes):
	...
	_0_test_bytes = bytearray((_block_length - _salt_length) + _block_length - n % _block_length - 1)
	_test_bytes = _0_test_bytes + _output[:n]

	for x in range(256):
		_x_byte = bytearray([x])
		if (encryption_oracle(_test_bytes + _x_byte)[_block_length:])[:_N_blocks*_block_length] == _0_padded_enc_bytes[len(_0_test_bytes) - (_block_length - _salt_length)][:_N_blocks*_block_length]:
